...
def get_session_messages(session_id):
    with _connect() as conn:
        rows = conn.execute(
            """
            SELECT role, content, domain, input_type, created_at
            FROM messages
            WHERE session_id=?
            ORDER BY id
            """,
...
def _auto_title_sessions(conn):
    """
    Automatically give old chats a title using
...
def get_history(limit=50):
    ...
    with _connect() as conn:
        ...
        # Repair old chats which still have "New Chat"
        _auto_title_sessions(conn)

        # Do not display completely empty chats
        rows = conn.execute(
            """
            SELECT
                s.id,
...
def analytics():
    ...
    with _connect() as conn:
        ...
        domains = conn.execute(
            """
            SELECT
                COALESCE(domain,'unknown') domain,
                COUNT(*) count
